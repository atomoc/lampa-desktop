...
function BrowserTestFun() {
    ...
    var extraToDisableClass =
        '.side_panel_movel,' +
        '.screen_size,' +
        '.side_panel_new_holder,' +
        '.screens,' +
        '.settings_holder,' +
        '.burn_in_protection,' +
    ...
    var enabledClicks = true,
        showClicks = enabledClicks ? 'cursor: pointer;' : 'pointer-events: none;',
        extraClass = enabledClicks ? '' : extraToDisableClass;

    var pointer_css_classes =
        '.hist_name,' +
    ...
    //css class from elem that need to show a mouse pointer
    Main_innerHTML(
        'pointer_css', //style elem
        pointer_css_classes //classes
    );
    //pointer-events: none;

    sidePanel_hover();
    controls_holder_hover();
    progress_pause_holder_hover();

    var sidePanel_element_show = Main_getElementById('side_panel_new_holder');
...
function BrowserTestSetPlayer(prop, value, prop2, value2, force_restart) {
    if (!embedPlayer || force_restart) {
        var obj = {
            ...
            muted: false,
            parent: ['localhost', 'twitch.tv', '127.0.0.1'],
            theme: 'dark'
        };
...
function BrowserTest_sidePanel_element_show() {
    var key = Main_values.Main_Go;
